...
const hasPrerequisites = prerequisites.every(prerequisite => finishedCourses.includes(prerequisite));

if (!hasPrerequisites) {
  // Get the prerequisites that the student has not completed
  const missingPrerequisites = prerequisites.filter(prerequisite => !finishedCourses.includes(prerequisite));

  // If the student has failed a course, suggest a new course to enroll in
  const failedCourse = course.prerequisites.find(prerequisite => !finishedCourses.includes(prerequisite));
  if (failedCourse) {
    const suggestedCourse = await Course.findOne({ prerequisites: failedCourse });
    if (suggestedCourse) {
  ...
  return next(new AppError(`You have not completed the following prerequisites: ${missingPrerequisites.join(', ')}`))};
